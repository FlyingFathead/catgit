...
def load_config():
    ...
    config.read_dict({'Defaults': {
        ...
        'display_omitted_files_in_tree_view': 'true'  # New configuration option
    }})
...
def get_all_git_ignored_files(path):
    ...
    ignored_files = set()
    try:
        # Use git ls-files to get ignored files
...
def is_text_file(file_path):
    ...
    try:
        ...
        if content:
            text_chars = bytes({7, 8, 9, 10, 12, 13, 27}) + bytes(range(0x20, 0x7f))
...
def concatenate_and_generate_tree(path, ignored_git_files, compiled_catgit_patterns, include_tree_view,
                                  ...
                                  display_omitted_files_in_tree_view=True):
        ...
        for index, entry in enumerate(entries):
            ...
            if os.path.isdir(full_path):
                ...
                if include_only and entry_relative_path not in included_dirs:
                    if display_omitted_files_in_tree_view:
                        tree_output += f"{prefix}{connector} # (omitted) {entry}/\n"
                        # Continue traversal to display the entire tree
...
def main():
    ...
    parser.add_argument('--include-only', action='store_true', help='Only include files listed in the include file')
    ...
    display_omitted_files_in_tree_view = config.getboolean('Defaults', 'display_omitted_files_in_tree_view')
    ...
    if include_tree_view or output_method == 'terminal':
        ...
        tree_view, concatenated_output = concatenate_and_generate_tree(
            ...
            display_omitted_files_in_tree_view=display_omitted_files_in_tree_view
        )
